...
const ZScheduledReminderAuditDataSchema = ZBaseRecipientDataSchema.extend({
  scheduledReminderId: z.string(),
...
export const ZDocumentAuditLogEventReminderCancelledSchema = z.object({
  type: z.literal(DOCUMENT_AUDIT_LOG_TYPE.REMINDER_CANCELLED),
  data: ZScheduledReminderAuditDataSchema,
});

export const ZDocumentAuditLogEventReminderFailedSchema = z.object({
  type: z.literal(DOCUMENT_AUDIT_LOG_TYPE.REMINDER_FAILED),
  data: ZScheduledReminderAuditDataSchema.extend({
    attemptCount: z.number().int().positive(),
    errorCode: z.string(),
...
export const ZDocumentAuditLogSchema = ZDocumentAuditLogBaseSchema.and(
  z.union([
    ...
    ZDocumentAuditLogEventReminderScheduledSchema,
