...
import { DocumentStatus, EnvelopeType, ScheduledReminderDeliveryStatus, WebhookTriggerEvents } from '@prisma/client';
    ...
    if (activeScheduledReminders.length > 0) {
      await tx.scheduledReminderDelivery.updateMany({
        ...
        data: {
          ...
          cancelledAt,
          claimedAt: null,
          lastErrorCode: 'DOCUMENT_CANCELLED',
          lastErrorMessage: 'The document was cancelled before the reminder was delivered',
        },
      });

      await tx.recipient.updateMany({
        where: { id: { in: activeScheduledReminders.map((delivery) => delivery.recipientId) } },
        data: {
