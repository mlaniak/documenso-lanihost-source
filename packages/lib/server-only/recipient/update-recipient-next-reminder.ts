...
export const updateRecipientNextReminder = async (options: {
  ...
  const automaticReminderAt = resolveNextReminderAt({
    config: settings,
    sentAt,
    lastReminderSentAt,
  ...
  const recipient = await prisma.recipient.findUnique({
    ...
    select: { scheduledReminderAt: true },
  });

  const nextReminderAt = getEarliestReminderAt(automaticReminderAt, recipient?.scheduledReminderAt);
...
export const recomputeNextReminderForEnvelope = async (envelopeId: string) => {
  ...
  const recipients = await prisma.recipient.findMany({
    ...
    select: { id: true, sentAt: true, lastReminderSentAt: true, reminderCount: true, scheduledReminderAt: true },
  });
