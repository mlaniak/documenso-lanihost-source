...
import { processScheduledReminderDelivery } from './process-scheduled-reminder-delivery';
...
export const run = async ({ payload, io }: { payload: TProcessSigningReminderJobDefinition; io: JobRunIO }) => {
  const { recipientId, scheduledReminderDeliveryId } = payload;
  const now = new Date();

  if (scheduledReminderDeliveryId) {
    await processScheduledReminderDelivery({ deliveryId: scheduledReminderDeliveryId, io });
    return;
  }

  // Atomically claim an automatic recurring reminder while preserving a later
  // manually scheduled resend.
  const updatedCount = await prisma.recipient.updateMany({
    where: {
      ...
      AND: [{ OR: [{ scheduledReminderAt: null }, { scheduledReminderAt: { gt: now } }] }],
      envelope: {
