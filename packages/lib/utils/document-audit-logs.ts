...
export const formatDocumentAuditLogAction = (i18n: I18n, auditLog: TDocumentAuditLog, userId?: number) => {
  ...
  const description = match(auditLog)
    ...
    .with({ type: DOCUMENT_AUDIT_LOG_TYPE.REMINDER_SENT }, ({ data }) => ({
      ...
      user: msg`${user}'s scheduled reminder was sent to ${data.recipientEmail}`,
    }))
    .with({ type: DOCUMENT_AUDIT_LOG_TYPE.REMINDER_CANCELLED }, ({ data }) => ({
      anonymous: msg`Scheduled reminder cancelled for ${data.recipientEmail}`,
      you: msg`You cancelled the scheduled reminder for ${data.recipientEmail}`,
      user: msg`${user} cancelled the scheduled reminder for ${data.recipientEmail}`,
    }))
    .with({ type: DOCUMENT_AUDIT_LOG_TYPE.REMINDER_FAILED }, ({ data }) => ({
      anonymous: msg`Scheduled reminder failed for ${data.recipientEmail}`,
