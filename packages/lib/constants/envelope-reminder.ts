...
export const getEarliestReminderAt = (...dates: Array<Date | null | undefined>): Date | null => {
  const availableDates = dates.filter((date): date is Date => date instanceof Date);

  if (availableDates.length === 0) {
    return null;
  }

  return new Date(Math.min(...availableDates.map((date) => date.getTime())));
};
